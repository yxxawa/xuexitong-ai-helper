import { SearchInformation } from './search.interface';
...
export interface SimplifyWorkResult {
	...
	searchInfos: {
		...
		/** 请求响应内容 */
		response?: SearchInformation['response'];
		/** 请求发起内容 */
		data?: SearchInformation['data'];
		/** 搜索错误信息 */
		error?: string;
		/** 搜索结果 [题目，答案，额外数据] */
...
export interface DefaultWork<E extends RawElements> {
	...
	/**
	 * 处理器，每个题目的处理器。可参考 packages/scripts/src/projects/cx.ts 中的作业/考试处理逻辑。
     *
     *
     * @param type 题目类型
...
export type WorkerEvents = {
	...
	close: () => void;
	/** 只补全未完成题 */
	completeUnfinished: (questionIndexes?: number[]) => void;
	/** 暂停答题 */
	stop: () => void;
	/** 继续答题 */
	continuate: () => void;

...
export function createDefaultQuestionResolver<E>(
	...
	return {
		...
		async single(infos, options, handler) {
			...
			const plainAnswer = getPlainAnswerFromAnswers(allAnswer, options.length, false);
			if (plainAnswer.length === 1) {
				const index = plainAnswer[0];
				await handler('single', options[index].innerText, options[index], ctx);
				return { finish: true, option: options[index], plainAnswer };
			}

			const semanticAnswers = allAnswer.filter((answer) => isPurePlainAnswer(answer) === false);
			let ratings: Rating[] = [];
			if (ctx.answerMatchMode === 'similar') {
				/** 配对选项的相似度 */
				ratings = answerSimilar(semanticAnswers, optionStrings);
				/**  找出最相似的选项 */
				let index = -1;
				let max = 0;
				let ans = '';
				...
				const result = answerExactMatch(semanticAnswers, optionStrings);
		...
		async multiple(infos, options, handler) {
			...
			const plainAnswer = getPlainAnswerFromAnswers(
				results.map((result) => result.answer),
				options.length,
				true
			);
			if (plainAnswer.length) {
				for (const index of plainAnswer) {
					await handler('multiple', options[index].innerText, options[index], ctx);
				}
				return { finish: true, plainOptions: plainAnswer.map((index) => options[index]), plainAnswer };
			}
			...
			for (let i = 0; i < results.length; i++) {
				...
				// 每个答案可能存在多个选项需要分割
				const answers = splitAnswer(result.answer.trim(), ctx.answerSeparators).filter(
					(answer) => isPurePlainAnswer(answer) === false
				);
		...
		async judgement(infos, options, handler) {
			for (const answers of infos.map((info) => info.results.map((res) => res.answer))) {
				const plainAnswerOption = answers
					.map((answer) => resolveJudgementPlainAnswer(answer, options.length))
					.find((option) => option !== undefined);
				if (plainAnswerOption !== undefined && options[plainAnswerOption]) {
					await handler('judgement', options[plainAnswerOption].innerText, options[plainAnswerOption], ctx);
					return { finish: true, option: options[plainAnswerOption], plainAnswerOption };
				}
				...
				function resolveJudgementPlainAnswer(answer: string, optionCount: number) {
					const text = String(answer || '').trim();
					const plainAnswer = clearString(text.replace(/^答案[:：]\s*/g, '').trim()).toUpperCase();
					const match = /^[A-Z]$/.test(plainAnswer)
						? plainAnswer
						: text.match(/(?:^|答案[:：]?\s*|选项?\s*)([A-Z])(?:$|[\s.．、,，:：])/i)?.[1]?.toUpperCase();
					if (match) {
						const index = match.charCodeAt(0) - 65;
...
function isPurePlainAnswer(answer: string) {
	return Boolean(resolvePlainAnswer(StringUtils.nowrap(String(answer || ''), '').trim()));
}

function getPlainAnswerFromAnswers(answers: string[], optionCount: number, allowMultiple: boolean) {
	for (const answer of answers) {
		const plain = resolvePlainAnswer(StringUtils.nowrap(String(answer || ''), '').trim());
		if (!plain) {
			continue;
		}
		if (!allowMultiple && plain.length !== 1) {
			continue;
		}
		const indexes = Array.from(plain)
			.map((char) => char.charCodeAt(0) - 65)
			.filter((index) => index >= 0 && index < optionCount);
		if (indexes.length === plain.length) {
			return indexes;
		}
	}
	return [];

...
公式答案支持常见的 LaTeX 反斜杠漏转义、JSON 代码块、尾随逗号及字符串内换行的本地解析修复，不因此增加 AI 请求。单个填空内容保留竖线、分号和换行；多个空优先按 `answers` 数组逐项填写，缓存也保留分空信息。无法可靠解析或被截断的 JSON 不会被当作整段答案填写；网页桥接仍必须匹配当前请求编号。

...
export class RemotePlaywright {
	...
	static async getRemotePage(
		...
		/**
		 * 桌面端后端无法拦截 GM_xmlhttpRequest ，所以这里使用 fetch 请求动作执行，然后后端根据key判断是否允许执行
		 */
		if (!this.authToken) {
			try {
	...
	private static createRemotePage(
		...
		for (const property of ListOfActions) {
			Reflect.set(page, property, async (...args: any[]) => {
				...
				try {
					// 这里为什么不写前缀 http://localhost:15319，因为有 Content-Security-Policy ， 这里我们借用后台的URL代理去进行处理，只要包含 xth-script-actions 即可轻松绕过 Content-Security-Policy 限制
					const res = await request('/xth-script-actions', {
						type: 'fetch',
						method: 'post',
